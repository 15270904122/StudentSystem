window.calendar = (function() {
    ...
    var W = (function() {
        ...
        function initId(){
            ...
            X = document.getElementById(opt.monthSelectId);
            G = G || document.getElementById(opt.heavenlyId);
            c = c || document.getElementById(opt.branchId);
            prev = document.getElementById(opt.prevId);
            next = document.getElementById(opt.nextId);
    ...
    var N = (function() {
        ...
        function C() {
            ...
            for (var i = 0; i < c.length; i ++ ) {
                ...
                if (c[i].solarDate == new Date().getDate()) {
                    mid = i
                }
            }
            var todate = Math.floor(mid / 7)
            forename = 'cell' + todate
            var tomonty = new Date().getMonth() + 1
            ...
            for (var a = 0; a < c.length; a++) {
                ...
                var index = Math.floor(a / 7);
                var X = R("DIV");
                // 选中的背景色
                // if (c[a].isToday) {
                //     today = Math.floor(c[a].solarDate/7)
                //     //选中cell的背景色和边框颜色
                //     X.style.cssText="background:#cde9d9;border-radius:50%;padding:5px 0;"
                //     // c[a].style.background = "#20C6BA";
                // }
                X.className = "cell" + index;
                X.style.left = (a % 7) * 42 / 100 + "rem";
                ...
                if (c[a].isToday) {
                    today = Math.floor(c[a].solarDate/7)
                    //选中cell的背景色和边框颜色
                    b.style.cssText="background: #fff;color: #263238 !important;border-radius: 50%; height: 28px;width: 26px; margin-left: 4px;"
                    // c[a].style.background = "#20C6BA";
                    b.className = "so hk";
                } else {
                    b.className = "so";
                }
                if (c[a].solarMonth == tomonty && Math.floor(a/7) == todate) {
                    if (bIndex == 0) { // 表示是第一个满足条件的
                       X.style.borderRadius = ".6rem 0 0 .6rem";
                    } else if (bIndex == 6) {
                       X.style.borderRadius = "0 .6rem .6rem 0";
                    } else if (a == nuIndex - 1) {
                       X.style.borderRadius = "0 .6rem .6rem 0";
                    }
                    X.style.background = '#20C6BA';
                    X.setAttribute("id", 'rowCell')
                    // aLi[i].style.background = "#20C6BA";
                    b.style.color = '#fff'
                    bIndex = bIndex + 1;
                } else {
                    b.style.color = '#263238'
                    // if (bIndex > 0) {
                    //     console.log('打算大苏打', c[a])
                    //     X.style.borderRadius = "0 .6rem .6rem 0"; 
                    // }
                }
    ...
    var F = (function() {
        ...
        //cell hover函数
        function G(b) {
            var name = b.cell.className
            var oUl = document.getElementById("cm");
            oUl.style.background = '#fff'; 
            var aLi = oUl.getElementsByClassName(name)
            for(i =0; i < aLi.length; i++){
                if(aLi[i].className && aLi[i].className == name){
                    aLi[i].style.background = "#20C6BA";
                    var children = aLi[i].children;
                    children[0].style.color = '#fff';
                    if (i == 0) {
                        aLi[i].style.borderRadius = ".6rem 0 0 .6rem"; 
                    }
                    if (i == aLi.length - 1) {
                        aLi[i].style.borderRadius = "0 .6rem .6rem 0"; 
                    }
                }
            } 
            var a = Q.getJson().dateArray[b.dateIndex];
            var Z = b.cell;
            Z.style.background = '#20C6BA';
        ...
        function I(b) {
            ...
            if(typeof fn === 'function'){
                ...
                var oUl = document.getElementById("cm");
                oUl.style.background = '#fff'; 
                if (forename != '' && name != forename) {
                    var bLi = oUl.getElementsByClassName(forename)
                    for(i =0; i < bLi.length; i++){
                        if(bLi[i].className && bLi[i].className == forename){
                            bLi[i].removeAttribute('id')
                            bLi[i].style.background = "#fff";
                            var children = bLi[i].children;
                            children[0].style.color = '#263238';
                            if (i == 0) {
                                bLi[i].style.borderRadius = ".6rem 0 0 .6rem"; 
                            }
                            if (i == bLi.length - 1) {
                                bLi[i].style.borderRadius = "0 .6rem .6rem 0"; 
                            }
                        }
                   } 
                }
